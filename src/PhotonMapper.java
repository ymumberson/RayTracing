...
//			System.out.println("light Ray:" + lightRay); //For testing
			traceLightRay(lightRay,rayPower,globalPhotons,true);
		}
...
//			System.out.println(target);
	...
	public void traceLightRay(Ray r, Vector rayPower, ArrayList<Photon> mapList) {
		traceLightRay(r, rayPower, mapList, false);
	}
	 ...
	 */
	public void traceLightRay(Ray r, Vector rayPower, ArrayList<Photon> mapList, boolean comesFromLightSource) {
		...
		if (t >= 0) { //Then there was an intersection
			...
			switch (hitTracable.russianRoulette(r, intersection)) {
				case 1: //Diffuse reflection
					...
					traceLightRay(newR,newPower,mapList);
					break;
				case 2: //Specular reflection
					...
					traceLightRay(newR,rayPower,mapList); //Power doesn't change
					break;
				case 3: //Refraction
					numRefract++;
...
//			System.out.println(rS + " " + rP + " " + cost + " " + cosi);
	...
	public Vector calculateIndirectIlluminationEstimate(Point intersection, Tracable currentTracable) {
		...
		PhotonMaxHeap diffuseHeap = new PhotonMaxHeap(NUM_PHOTONS_SEARCHING_FOR);
		for (int i=0; i<NUM_PHOTONS_SEARCHING_FOR; i++) {
			Photon p2 = globalPhotons.get(i);
			diffuseHeap.insert(p2, intersection.euclideanDistance(p2.getPosition()));
		}
		globalMap.getNearestNeighbours(intersection,diffuseHeap);
		float k = 1;
		diffuse = diffuseHeap.getAverageColourConeFilter(k).multiply(new Vector(currentTracable.getColor()));
